...
class FeatureList extends Component {
    getColumnList() {
        ...
        let columnList = columns.map((column, index) => {
            ...
            return (<FeatureItem key={`${columnName}-${index}`} tid={tid} text={columnName} selected={selected} click={clickItem} checkbox={checkbox}/>);
        });
        return columnList;
    }
    ...
    render() {
        return (<div className="feature-div">
            ...
            <ol className="feature-list">{this.getColumnList()}</ol>
        </div>);
    }
}
